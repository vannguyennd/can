...
def compare_target_predict_scope(start_predicts, end_predicts, start_targets, end_targets, predict_combine,
                                 target_combine):

    target_set = set(zip(start_targets, end_targets))
    predict_set = set(zip(start_predicts, end_predicts))

    c_true_check = target_set.union(predict_set)
    c_false = len(target_set - predict_set)

    c_true = 0
    for i_pair in c_true_check:
        target = target_combine[i_pair[0]:i_pair[1] + 1]
        predict = predict_combine[i_pair[0]:i_pair[1] + 1]
        if compare(target, predict):
            c_true += 1
...
def result_target_predict_scope_recall_2(dir_path, step):
    ...
    count_true, count_false = compare_target_predict_scope(start_predicts, end_predicts, start_targets, end_targets,
                                                           predict_combine, target_combine)
...
def result_target_predict_scope_precision_2(dir_path, step):
    ...
    count_true, count_false = compare_target_predict_scope(start_targets, end_targets, start_predicts, end_predicts,
                                                           predict_combine, target_combine)
